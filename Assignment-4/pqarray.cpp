...
void PQArray::expand() {
    int new_numAllocated = _numAllocated * 2;
    DataPoint* new_elements = new DataPoint[new_numAllocated]();
    for (int i = 0 ; i < _numAllocated ; i++) {
        new_elements[i] = _elements[i];
    }
    _elements = new_elements;
    _numAllocated = new_numAllocated;
}
...
void PQArray::enqueue(DataPoint elem) {
    ...
    if (size() == _numAllocated) expand();
    int pos = find(elem.priority);
    _numFilled++;
...
int PQArray::find(double priority) {
    int left = 0, right = _numFilled - 1;
    while(left <= right) {
        int mid = (right + left) / 2;
        double v = _elements[mid].priority;
        if (v < priority) {
            right = mid - 1;
        }else if(v > priority){
            left = mid + 1;
