...
void validatePath(Grid<bool> &maze, Stack<GridLocation> path) {
  ...
  Set<GridLocation> visist = {cur};
  while (true) {
    if (!moves.contains(path.peek())) error("error path");
    cur = path.pop();
    if (visist.contains(cur)) {
      error("revisit");
    }
    visist.add(cur);
    moves = generateValidMoves(maze, cur);
    if (path.isEmpty()) {
      if (cur.col != 0 || cur.row != 0) error("error start position");
      return;
    }
  }
